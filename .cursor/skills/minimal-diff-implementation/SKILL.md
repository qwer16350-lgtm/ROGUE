...
1. 플랜에 없는 파일: **수정 금지**
2. **Surgical changes:** 승인 범위 안에서만 줄 단위로 최소 수정. “더 깔끔해 보인다”는 이유만으로 패턴·추상화·파일 분리를 추가하지 않는다.
3. “리팩터/정리/스타일” **드라이브**: 금지 (플랜에 있을 때만)
4. 한 커밋(또는 한 턴)의 이야기는 **한 목적**만
5. 엔트리(`MainServer`, `MainClient`)는 **wiring만** — 비대한 로직 직접 삽입 지양 (플랜 달리면 예외)
